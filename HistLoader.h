...
class HistLoader
{
  std::vector<std::string> fnamevec[3];
  std::vector<TFile*> filevec[3];
  std::vector<int>    runvec[3];
  TFile* fdummy;
  char dummyname[500];
...
public:
  ...
  std::vector<TTree*> GetTreeVector(char* name, int charge, int runmin, int runmax);
  std::vector<TTree*> GetTreeVector(char* name) {return GetTreeVector(name,0,0,10000000);}
  std::vector<TTree*> GetTreeVector(char* name, int charge) {return GetTreeVector(name,charge,0,10000000);}
  std::vector<TTree*> GetTreeVector(char* name, int runmin, int runmax) {return GetTreeVector(name,0,runmin,runmax);}
};
